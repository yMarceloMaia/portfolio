...
const Card = ({ project }: PropsProjects) => {
    ...
    return (
        <div className={`card border rounded-lg overflow-hidden shadow-lg ${cardClasses}`}>
            ...
            <div className="p-6">
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                    {project.technologies.map((tech, index) => (
                        <span key={index} className={`px-2 py-1 rounded-full text-sm ${techClasses}`}>{tech}</span>
                    ))}
                </div>
                <div className="flex justify-between">
                    ...
                    <a href={project.repository} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:text-blue-400">GitHub</a>
                </div>
            </div>
        </div>
    )
